...
export async function createCoworkingSpace(formState: unknown, formData: FormData) {
  ...
  if (validatedFields.success) {
    await dbConnect();
    try {
      const coworkingSpace = await CoworkingSpace.create(validatedFields.data);
      if (coworkingSpace) {
...
export async function editCoworkingSpace(formState: unknown, formData: FormData) {
  ...
  if (!session || !id || session.user.role != "admin") return { success: false };
  const data = Object.fromEntries(formData.entries());
  const validatedFields = await EditCoworkingSpaceForm.safeParseAsync(data);
  await dbConnect();
  if (validatedFields.success) {
    try {
      const coworkingSpace = await CoworkingSpace.findByIdAndUpdate(id, validatedFields.data, {
        new: true,
        runValidator: true,
      });
      if (coworkingSpace) {
...
export async function deleteCoworkingSpace(formState: unknown, formData: FormData) {
  const session = await auth();
  const id = formData.get("id")?.toString();
  if (!session || !id || session.user.role != "admin") return { success: false };
  await dbConnect();
